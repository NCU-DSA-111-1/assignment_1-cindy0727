...
int main(void)
{
    ...
    for(i=0;i<num_layers;i++)
    {
        if(i==0)
        {
            printf("Enter number of input bits(layer[%d]): \n",i+1);
            scanf("%d",&num_neurons[i]);
        }
        else if(i==num_layers-1)
        {
            printf("Enter number of output bits(layer[%d]): \n",i+1);
            scanf("%d",&num_neurons[i]);
        }
        else
        {
    ...
    printf("Enter the number of training examples(2^input bits):\n");
    ...
    if((cfptr=fopen("loss.csv","w"))==NULL)
    {
